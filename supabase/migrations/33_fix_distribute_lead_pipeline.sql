-- Migration 33: Fix distribute_lead() preserving existing pipeline when rule has no pipeline.
-- Bug in migration 31: SET pipeline_id = v_rule.pipeline_id would set it to NULL
-- when the distribution rule has no pipeline, clearing the lead's current pipeline.
-- Fix: use coalesce(v_rule.pipeline_id, pipeline_id) to keep existing value.
