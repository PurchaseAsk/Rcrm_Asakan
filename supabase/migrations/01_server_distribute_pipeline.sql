-- Keep server-side lead distribution aligned with the pipeline-aware client flow.
-- This replaces the original function from 001_init.sql.
...
begin
  ...
  values (
    ...
    'assigned',
    format(
      'Auto distributed [%s] pipeline=%s assignee=%s',
      v_rule.method,
      coalesce(v_rule.pipeline_id::text, '-'),
      coalesce(v_assign::text, '-')
    )
  );
end
$$;
